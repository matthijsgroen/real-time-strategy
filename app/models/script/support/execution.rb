module Script::Support::Execution
	...
	module InstanceMethods
		...
		def load_settings_from(hash = {})
			self.parameters ||= {}
			self.parent_id = hash[:parent].id if hash[:parent]
			self.parent_id ||= hash[:parent_id]

			if hash[:parent] or hash[:parent_id]
				p = hash[:parent] || Script::Base.find(hash[:parent_id])
				self.faction_id = p.faction_id
				self.game_instance_id = p.game_instance_id
			end

			if hash[:asset] or hash[:asset_id]
				a = hash[:asset] || Asset::Base.find(hash[:asset_id].to_i)
				self.faction_id = a.faction_id
				self.game_instance_id = a.game_instance_id
			end

			self.faction_id = hash[:faction].id if hash[:faction]
			self.faction_id = hash[:faction_id] if hash[:faction_id]

			self.game_instance_id ||= hash[:game_instance_id]
			self.game_instance_id = hash[:game_instance].id if hash[:game_instance]
			if not self.game_instance_id and self.faction_id
				f = hash[:faction] || Faction.find(self.faction_id)
				self.game_instance_id = f.game_instance_id
			end
			self.start_time = hash[:start_time] || Time.now.utc
		...
		def execute(*args)
			...
			if options[:initiated_by]
				script = find_or_initialize_by_initiated_by_id_and_initiated_by_type(
					options[:initiated_by].id,
					options[:initiated_by].class.name)
			else
				script = self.new
			end
			script.position = 0
