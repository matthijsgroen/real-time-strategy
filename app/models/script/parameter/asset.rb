	...
	def unserialize_asset(parameter, parameters)
		...
		[parameter, options[:alias]].compact.each do |param|
			...
			if possible_id = parameters[param.to_id] and not assigned
				begin
					assigned = Asset::Base.find_with_deleted possible_id
				rescue
				end
			end
		end
		raise "#{self.inspect}: No Asset found for required parameter :#{parameter} is the option :alias set? tried ID #{possible_id}" if not assigned and options[:required]
